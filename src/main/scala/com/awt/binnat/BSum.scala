...
trait BSumImpl extends BSumLowLevelImpl with BSumMem1Impl {
  ...
  implicit object _1_plus_1 extends BNonZeroSum[_1, _1] {
    final type Out = _1 ## _0
  }

  implicit def _0_plus_n[N <: BNonZero, D <: BDigit] =
    new BNonZeroSum[_0, N ## D] {final type Out = N ## D}

  implicit def _n_plus_0[N <: BNonZero, D <: BDigit] =
    new BNonZeroSum[N ## D, _0] {final type Out = N ## D}

  implicit def _1_plus_n0[N <: BNonZero] =
    new BNonZeroSum[_1, N ## _0] {final type Out = N ## _1}

  implicit def _1_plus_n1[N <: BNonZero](implicit higher: BNonZeroSum[_1, N]) =
    new BNonZeroSum[_1, N ## _1] {final type Out = higher.Out ## _0}
  ...
  (implicit digitSum: BDigitSum[Da, Db], higher: BNonZeroSum[Na, Nb]) =
    new BNonZeroSum[Na ## Da, Nb ## Db] {final type Out = higher.Out ## digitSum.Out}
...
trait BSumMem1Impl {
  ...
  (implicit higher: BSumMem1[Na, Nb], digitSum: BNZDigSumMem1[Da, Db]) =
    new BSumMem1[Na ## Da, Nb ## Db] {final type Out = higher.Out ## digitSum.Digit}
}
